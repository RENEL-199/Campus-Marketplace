...
class OrderRepository {
    ...
    public function placeOrder(int $userId, array $cartItemIds, string $fullname, string $address, string $phone, string $paymentMethod, ?string $paymentProofPath = null, bool $rentalTermsAccepted = false): int {
        ...
        try {
            ...
            if (!empty($paymentProofPath)) {
                $this->pdo->prepare("UPDATE orders SET payment_proof_path = ? WHERE order_id = ?")->execute([$paymentProofPath, $orderId]);
            }
