...
class ProductRepository {
    ...
    public function add(Product $product): void {
        ...
        $stmt = $this->pdo->prepare("
            ...
            (user_id, category_id, prod_name, prod_desc, prod_price, prod_image, prod_stock, location, rate_type, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
        ");
        $stmt->execute([
            $product->user_id,
    ...
    public function update(Product $product): void {
        $stmt = $this->pdo->prepare("
            UPDATE products SET
                ...
                prod_image = ?, prod_stock = ?, location = ?, rate_type = ?
            WHERE prod_id = ? AND user_id = ?
        ");
        $stmt->execute([
            $product->category_id, $product->prod_name, $product->prod_desc, $product->prod_price,
            $product->prod_image, $product->prod_stock, $product->location, $product->prod_rate_type,
